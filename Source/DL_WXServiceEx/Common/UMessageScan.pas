...
begin
  ...
  try
    ...
    try
      ...
      if FNumOutFactMsg = 0 then DoSaveOutFactMsg;
      ///******************************************************
      {$IFDEF WXCreateOrderCtrl}
      //  读取微信服务端已开订单 便于 限量开单控制
      if FNumOutFactMsg = 1 then GetWXOrders;      //Continue;
      {$ENDIF}
      ///******************************************************
...
procedure TMessageScanThread.GetWXOrders;
var nInit: Int64;
    nstr : string;
    nOut: TWorkerBusinessCommand;
begin
  WriteLog('定时读取工厂微信已开订单');
  nInit := GetTickCount;
  nstr:= PackerEncodeStr('ss');//'<?xml version="1.0" encoding="UTF-8"?><DATA><Head><Command>1328</Command><RemoteUL>127.0.0.1</RemoteUL></Head></DATA>';
  TBusWorkerBusinessWebchat.CallMe(cBC_WX_GetCustomerWxOrders,
                       nstr,'',@nOut);

  WriteLog('定时读取工厂微信已开订单耗时: ' + IntToStr(GetTickCount - nInit) + 'ms');
end;
